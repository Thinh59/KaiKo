...
- **Chấm điểm cuối trận**: `gemini-3.1-flash-lite-preview` phân tích toàn bộ transcript → cho điểm logic, dẫn chứng, cách diễn đạt → hiển thị Scoreboard chi tiết
- **Lưu lịch sử**: Kết quả tự động lưu vào PostgreSQL (Neon cloud) sau mỗi trận

### Use Case 2 – Tranh biện Text 1v1 (Chat-based)
- **Matchmaking text mode**: Ghép cặp riêng cho chế độ `text_1v1`, tự động phân chia ngẫu nhiên phe Ủng hộ / Phản đối.
- **Giới hạn thời gian**: Thời gian tranh biện mở rộng lên đến 30 phút, tự động kết thúc và chấm điểm khi hết giờ.
- **Giao diện chat luân phiên**: Player A và B gõ lập luận xen kẽ.
- **AI phân tích inline**: Mỗi lượt gõ xong → backend phân tích ngụy biện.
- **Solo vs AI (text)**: Người chơi tranh biện với AI (không cần đối thủ).
...
1. Download model từ Drive: [model](https://drive.google.com/drive/folders/1n_xn3Ms4VPRW3aoYshsSCEOfa2nS5IJi?usp=sharing)
...
| Model local | Model XLM-RoBERTa (~400MB) chưa được commit lên Git; cần download riêng từ Drive |
